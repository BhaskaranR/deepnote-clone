...
import InviteEmail from "@deepnote-clone/email/emails/invite";
import { getI18n } from "@deepnote-clone/email/locales";
import { LogEvents } from "@deepnote-clone/events/events";
import { client as RedisClient } from "@deepnote-clone/kv/client";
import type { Database } from "@deepnote-clone/supabase/db";
  ...
  .action(
    async ({
      ...
      const emails = invtesData?.map(async (invites) => {
        ...
        const team =
          invites.team as Database["public"]["Tables"]["team"]["Row"];
        return {
          from: "Deepnote Clone <support@deepnote-clone.com>",
          to: [invites.email],
